...
import { FormSchemaService } from 'data/services/FormSchemaServive';
import {
  CadastroClienteFormDataInterface,
  CredenciaisInterface,
  LoginFormDataInterface,
  NovaDiariaFormDataInteface,
  PagamentoFormDataInterface,
} from 'data/@types/FormIterface';
...
export default function useContratacao() {
  const [step, setStep] = useState(1),
    ...
    serviceForm = useForm<NovaDiariaFormDataInteface>({
    ...
    paymnetForm = useForm<PagamentoFormDataInterface>({
  ...
  useEffect(() => {
    //verificar disponibilidade do atendimento
  ...
  function onServiceFormSubmit(data: NovaDiariaFormDataInteface) {
  ...
  async function onPaymentFormSubmit(data: PagamentoFormDataInterface) {
    ...
    ApiServiceHateoas(novaDiaria.links, 'pagar_diaria', async (request) => {
      try {
        ...
        paymnetForm.setError('pagamento_recusado', {
          type: 'manual',
  ...
  function calcularTempoServico(
    ...
    let total = 0;
    console.log(dadosFaxina, tipoLimpeza);
